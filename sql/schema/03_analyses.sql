-- Tables d'analyse (cf. L011 §8, L030.3) — immuables après création (cf. ADR-002 de L001).
...
CREATE INDEX IF NOT EXISTS idx_analyse_course ON analyse (course_id);
CREATE INDEX IF NOT EXISTS idx_analyse_score ON analyse (score_confiance);
CREATE INDEX IF NOT EXISTS idx_analyse_date ON analyse (date_calcul);

CREATE TABLE IF NOT EXISTS analyse_partant (
    id               BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    analyse_id       BIGINT NOT NULL REFERENCES analyse(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS controle_roi (
    id             BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    analyse_id     BIGINT NOT NULL REFERENCES analyse(id) ON DELETE RESTRICT,
    mise           DECIMAL(8,2) NOT NULL DEFAULT 0,
